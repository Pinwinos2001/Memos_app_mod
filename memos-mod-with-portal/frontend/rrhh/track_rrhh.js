...
function render(){
  ...
  if(!rows || rows.length===0){
    var msg = currentView==='pending' ? 'Sin memos por aprobar' 
            : currentView==='approved' ? 'Sin memos aprobados/emitidos'
  ...
  tbody.innerHTML = rows.map(function(m){
    ...
    if(currentView==='pending'){
      acc = '<a class="btn btn-primary" href="/rrhh/review.html?id='+encodeURIComponent(m.id)+'" target="_blank">Revisar</a>';
    }
    return '<tr>'
      + '<td><b>'+(m.memo_id||'')+'</b></td>'
      + '<td>'+(m.dni||'')+'</td>'
...
function cargar(){
  ...
  authFetch('/api/summary?role=rrhh')
    ...
    .then(function(data){
      console.log('API Response (RRHH):', data); // Debug
      ...
      console.log('Cache actualizado (RRHH):', cache); // Debug
      render();
    })
    .catch(function(err){
      if(String(err) !== 'Error: 401'){
        tbody.innerHTML = emptyRow('Error al cargar datos');
        console.error('Error en cargar() RRHH:', err);
      }
    })
    .finally(function(){
